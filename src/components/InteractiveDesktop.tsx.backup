  ...
  return (
    <footer style={wrapperStyle} data-name="Bloco 08" data-node-id="7:249" role="contentinfo" aria-label="Rodapé">
      ...
      <div style={contentStyle}>
        ...
        {/* Logo Setima - posicionamento exato do Figma com funcionalidade de clique */}
        <button 
          onClick={() => {
            window.scrollTo({ top: 0, behavior: 'smooth' });
          }}
...
export default function InteractiveDesktop({ scale = 1 }: { scale?: number }) {
  ...
  // Controle suave de scroll sem "tremida"
  useEffect(() => {
    let maxScrollY = 0;
    ...
    // Controlar scroll via CSS em vez de JavaScript para evitar conflitos
